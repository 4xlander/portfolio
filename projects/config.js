...
const projectsConfig = [
    {
        id: 'proj-17',
        icon: '💼',
        image: 'projects/proj-17/cover.jpg',
        title: {
        ...
        genreTags: [],
        detailedDescription: {
            ru: "<div class=\"project-meta\"><p><strong>Роль:</strong> Unity-разработчик (соло)</p><p><strong>Платформа:</strong> Mobile</p><p><strong>Технологии:</strong> Unity UI, Unity Ads, PrimeTween, VContainer</p></div><h4>Описание:</h4><p>Полноценная 2D игра, выполненная по техническому заданию заказчика.<br>Игрок в роли пирата атакует острова. Для захвата необходимо успешно пройти мини-игру. В случае успеха игрок усиливает свой экипаж матросами, найденными на острове, и получает возможность атаковать более сложные цели.</p><p>Захват острова:</p><ul class=\"tasks-list\"><li><span>Мини-игра состоит из нескольких уровней, которые игрок проходит последовательно, открывая ячейки на поле</span></li><li><span>Для завершения уровня необходимо найти ключ, открывающий ворота, либо артефакт «Отмычка»</span></li><li><span>На пути могут встретиться монстры, блокирующие проход — тап по монстру наносит урон, но если враг выживает, он сразу отвечает атакой</span></li><li><span>Если у героя заканчивается здоровье, захват острова проваливается, и прохождение нужно начинать заново</span></li><li><span>На уровнях встречаются артефакты с различными свойствами: одни дают пассивные бонусы, другие необходимо использовать вручную</span></li><li><span>Артефакты могут лежать на поле или выпадать из монстров. Все найденные предметы сохраняются на протяжении текущего рейда</span></li><li><span>Игрок может продавать артефакты, освобождая ячейки инвентаря. Золото, как и артефакты, сохраняется только в пределах рейда</span></li><li><span>Иногда на уровне встречается магазин с бонусами и артефактами, которые можно купить за накопленное золото</span></li><li><span>Дополнительные ячейки инвентаря можно разблокировать, просмотрев рекламный ролик</span></li></ul>",
            en: "<div class=\"project-meta\"><p><strong>Role:</strong> Unity Developer (solo)</p><p><strong>Platform:</strong> Mobile</p><p><strong>Technologies:</strong> Unity UI, Unity Ads, PrimeTween, VContainer</p></div><h4>Description:</h4><p>A complete 2D game developed according to the client’s technical requirements.<br>The player takes on the role of a pirate attacking islands. To capture an island, the player must successfully complete a mini-game. Upon success, the player strengthens their crew with sailors found on the island and gains the ability to attack more challenging targets.</p><p>Island Capture:</p><ul class=\"tasks-list\"><li><span>The mini-game consists of several levels that the player progresses through by sequentially opening tiles on the board</span></li><li><span>To complete a level, the player must find a key that opens the gate, or an artifact called the “Lockpick”</span></li><li><span>Monsters may appear and block the path — tapping on a monster deals damage, but if it survives, it immediately counterattacks</span></li><li><span>If the hero’s health reaches zero, the island capture attempt fails and must be restarted</span></li><li><span>Various artifacts can be found on levels: some provide passive bonuses, while others must be used manually</span></li><li><span>Artifacts can be found on the ground or dropped by monsters. All collected items remain with the player throughout the current raid</span></li><li><span>The player can sell artifacts to free up inventory slots. Gold, like artifacts, is preserved only within the current raid</span></li><li><span>Occasionally, a shop may appear on a level, offering bonuses and artifacts purchasable with collected gold</span></li><li><span>Additional inventory slots can be unlocked by watching a rewarded ad</span></li></ul>"
        },
        media: {
            type: 'youtube',
            url: 'https://youtu.be/ZVwdESUUeeg'
        }
    },
    {
        id: 'test-project-3',
        icon: '💼',
        image: 'projects/test-project-3/cover.jpg',
        title: {
            ru: 'Тестовый проект',
            en: 'Test project'
        },
        tags: ['Personal', 'Test', '2D', 'Git'],
        genreTags: ['Interactive inventory'],
        detailedDescription: {
            ru: "<div class=\"project-meta\"><p><strong>Технологии: </strong>Unity, MVP, Drag & Drop</p></div><h4>Описание:</h4><p>Система интерактивного инвентаря с предметами произвольной формы и генерацией ресурсов.<br>Игровое поле представляет собой сетку (по умолчанию 8×8 клеток, можно изменить через конфиг), в которую игрок может перемещать предметы из стартовой области с помощью Drag & Drop.<br>Каждый предмет имеет форму, аналогичную фигурам из «Тетриса», и одну главную клетку, производящую ресурсы в зависимости от типа тайла, на котором она расположена.<br>Типы тайлов инвентаря генерируются по заданному алгоритму.<br>Предметы в стартовой области создаются случайно — количество и набор возможных задаются через конфиг.<br>Можно приобретать артефакты с различными эффектами.<br>Логика построена по архитектурному паттерну MVP, обеспечивая чёткое разделение данных, представления и поведения.</p><p>Основные возможности:</p><ul class=\"tasks-list\"><li><span>Перетаскивание предметов из стартовой зоны в инвентарь</span></li><li><span>Поворот предмета правой кнопкой мыши</span></li><li><span>Замещение и возврат предметов при пересечении</span></li><li><span>Уничтожение предмета с созданием нового случайного и затратой ресурсов</span></li><li><span>Производство трёх видов ресурсов (Пшеница, Дерево, Железо)</span></li><li><span>Разные типы тайлов (зелёный, жёлтый, красный) с различной скоростью производства</span></li><li><span>Анимация и попап при каждом произведённом ресурсе</span></li><li><span>Изменение скорости игры (×1, ×2, ×3)</span></li><li><span>Система артефактов с уникальными эффектами и ростом стоимости при каждой покупке</span></li></ul>",
            en: "<div class=\"project-meta\"><p><strong>Technologies: </strong>Unity, MVP, Drag & Drop</p></div><h4>Description:</h4><p>An interactive inventory system with irregularly shaped items and resource generation.<br>The game field is a grid (8×8 cells by default, configurable) where the player can move items from the starting area using Drag & Drop.<br>Each item has a unique shape similar to Tetris pieces and a main cell that produces resources depending on the type of tile it’s placed on.<br>Inventory tile types are generated according to a specified algorithm.<br>Items in the starting area are generated randomly — the amount and possible types are defined in the config.<br>Players can purchase artifacts with various effects.<br>The logic is implemented using the MVP architectural pattern, ensuring a clear separation of data, presentation, and behavior.</p><p>Main features:</p><ul class=\"tasks-list\"><li><span>Drag & Drop items from the starting zone into the inventory</span></li><li><span>Rotate items with the right mouse button</span></li><li><span>Replace or return items when overlapping occurs</span></li><li><span>Destroy an item to create a new random one at the cost of resources</span></li><li><span>Produce three types of resources (Wheat, Wood, Iron)</span></li><li><span>Different tile types (green, yellow, red) with varying production speeds</span></li><li><span>Animated popups for each produced resource</span></li><li><span>Adjustable game speed (×1, ×2, ×3)</span></li><li><span>Artifact system with unique effects and increasing cost with each purchase</span></li></ul>"
        },
        links: [
            { text: { ru: 'GitHub', en: 'GitHub' }, url: 'https://github.com/4xlander/test-project-3' }
        ]
    },
    {
        id: 'proj-16',
        ...
        tags: ['Commercial', 'Solo', '2D', 'Mobile'],
        genreTags: ['Tower Defense Lite'],
        detailedDescription: {
            ru: "<div class=\"project-meta\"><p><strong>Роль:</strong> Unity-разработчик (соло)</p><p><strong>Платформа:</strong> Mobile</p><p><strong>Технологии:</strong> Unity UI, 2D Physics, Unity In-App, Unity Ads, PrimeTween, VContainer </p></div><h4>Описание:</h4><p>Полноценная 2D игра в жанре Tower Defense Lite, выполненная по техническому заданию заказчика.</p><p>Основной геймплей:</p><ul class=\"tasks-list\"><li><span>Монстры нападают на игрока волнами, стремясь разрушить защитное заграждение</span></li><li><span>С каждой новой волной увеличивается как количество монстров, так и их характеристики</span></li><li><span>Игрок защищает заграждение набором луков, собранных до начала битвы</span></li><li><span>За уничтожение монстров игрок получает монеты, которые можно потратить на усиление системы защиты между сражениями</span></li><li><span>Битва заканчивается, когда монстры уничтожают заграждение</span></li></ul><p>Режимы управления луками в бою:</p><ul class=\"tasks-list\"><li><span>Луки автоматически атакуют цель, попавшую в их радиус авто-атаки</span></li><li><span>Игрок может тапом выделить конкретного врага — тогда все луки начнут атаковать его независимо от расстояния</span></li><li><span>Игрок может тапом выбрать лук, а затем врага, задав приоритетную цель конкретному луку</span></li><li><span>Тап по пустому игровому полю переводит все луки обратно в режим авто-атаки</span></li></ul><p>Между битвами игрок усиливает систему защиты:</p><ul class=\"tasks-list\"><li><span>Улучшает луки, открывает и покупает новые</span></li><li><span>Расширяет набор, открывая дополнительные ячейки</span></li><li><span>Формирует набор луков в соответствии со своей стратегией</span></li></ul><p>В игре представлено несколько видов луков с разными характеристиками и эффектами. Для каждого лука можно прокачивать такие параметры, как урон, скорострельность и радиус авто-атаки. Каждый параметр улучшается отдельно.</p><p>Каждый вид имеет собственное дерево прогрессии: для доступа к луку следующего ранга необходимо полностью прокачать предыдущий.</p><p>Дополнительный функционал:</p><ul class=\"tasks-list\"><li><span>Ежедневные награды в виде монет</span></li><li><span>Ненавязчивая реклама в формате баннеров</span></li><li><span>Внутриигровые покупки: разовые наборы, монеты, отключение рекламы</span></li></ul>",
            en: "<div class=\"project-meta\"><p><strong>Role:</strong> Unity Developer (solo)</p><p><strong>Platform:</strong> Mobile</p><p><strong>Technologies:</strong> Unity UI, 2D Physics, Unity In-App, Unity Ads, PrimeTween, VContainer </p></div><h4>Description:</h4><p>A complete 2D game in the Tower Defense Lite genre, developed according to the client’s technical requirements.</p><p>Main gameplay:</p><ul class=\"tasks-list\"><li><span>Monsters attack the player in waves, attempting to destroy the defensive barrier</span></li><li><span>With each new wave, both the number of monsters and their stats increase</span></li><li><span>The player defends the barrier using a set of bows prepared before the battle</span></li><li><span>Defeating monsters rewards the player with coins, which can be used to upgrade defenses between battles</span></li><li><span>The battle ends when monsters destroy the barrier</span></li></ul><p>Bow control modes during battle:</p><ul class=\"tasks-list\"><li><span>Bows automatically attack any enemy that enters their auto-attack radius</span></li><li><span>The player can tap a specific enemy to make all bows target it regardless of distance</span></li><li><span>The player can tap a bow and then an enemy to assign that enemy as a priority target for the selected bow</span></li><li><span>Tapping on empty ground switches all bows back to auto-attack mode</span></li></ul><p>Between battles, the player can strengthen their defenses:</p><ul class=\"tasks-list\"><li><span>Upgrade bows, unlock and purchase new ones</span></li><li><span>Expand the set by unlocking additional slots</span></li><li><span>Assemble a bow setup according to their defense strategy</span></li></ul><p>The game features several types of bows with unique stats and applied effects. Each bow can be upgraded in separate parameters: damage, attack speed, and auto-attack radius.</p><p>Each bow type has its own progression tree: to unlock a higher-rank bow, the previous rank must be fully upgraded.</p><p>Additional functionality:</p><ul class=\"tasks-list\"><li><span>Daily rewards in the form of coins</span></li><li><span>Non-intrusive banner ads</span></li><li><span>In-app purchases: one-time packs, coins, ad removal</span></li></ul>"
        },
        media: {
            type: 'youtube',
            url: 'https://youtube.com/shorts/qNjdFQ8UtM0'
        }
    },
    {
        id: 'test-minesweeper',
    ...
    },
    {
        ...
        media: {
            type: 'youtube',
            url: 'https://youtu.be/Pk6Ml3wPk_0'
        },
        links: [
